...
import { getAgencyById, getCashById, getEnterpriseById, queryClient, updateCash } from "../../utils/http";
...
export default function UpdateCash() {
    ...
    useEffect(() => {
        async function get(signal) {
            if (user.role.includes("ROLE_ADMIN") && id != "" || user.role.includes("ROLE_COMPTABLE") && id != "") {
                ...
                const enterprise = await getEnterpriseById({ id: cash.enterprise, signal })
                const agency = await getAgencyById({ id: cash.agency, signal })
                setData(prev => {
                    return {
                        ...prev,
    ...
    async function handleSubmit(prevState, formData, signal) {
        ...
        mutate({ slug: cash.slug, cashDto: allData })
        return { errors: null }
    }
