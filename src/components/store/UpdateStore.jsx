...
export default function UpdateStore() {
    ...
    return <>
        ...
        <form action={formAction} className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 rounded-lg text-sky-50 p-4" ref={scope}>
            ...
            {data?.store?.name !== "CLIENT" && data?.store?.name !== "REBUTS" && user.role.includes("ROLE_COMPTABLE") && data?.store?.name !== "CLIENT" && data?.store?.name !== "REBUTS" && user.role.includes("ROLE_GESTIONNAIRE_DE_STOCK") ? < Submit >
                Enregistrer
            </Submit> : undefined}
        </form >
